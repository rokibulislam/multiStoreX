...
if ( ! function_exists( 'multistorex_get_order_report_data' ) ) :
	/**
	 ...
	 * @return obj
	 */
	function multistorex_get_order_report_data( $args, $start_date, $end_date ) {
		global $wpdb;

		$current_user = multistorex_get_current_user_id();
		...
		foreach ( $data as $key => $value ) {
			...
			} elseif ( $value['type'] == 'multistorex_orders' ) {
				$get_key = "do.{$key}";
			}
		...
		// Joins
		$joins         = array();
		$joins['do']  = "LEFT JOIN {$wpdb->prefix}multistorex_orders AS do ON posts.ID = do.order_id";
		...
		$query      = apply_filters( 'multistorex_reports_get_order_report_query', $query );
		$query      = implode( ' ', $query );
		...
		if ( $debug || $nocache || ( false === $result ) ) {
			$result = apply_filters( 'multistorex_reports_get_order_report_data', $wpdb->$query_type( $query ), $data );
...
if ( ! function_exists( 'multistorex_dashboard_sales_overview' ) ) :
	 ...
	 * @return void
	 */
	function multistorex_dashboard_sales_overview() {
		...
		multistorex_sales_overview_chart_data( $start_date, $end_date, 'day' );
	}

endif;


if ( ! function_exists( 'multistorex_sales_overview_chart_data' ) ) :
     ...
     * @return void
	 */
	function multistorex_sales_overview_chart_data( $start_date, $end_date, $group_by ) {
		global $wp_locale;

        $now                = multistorex_current_datetime();
		$start_date_to_time = $now->modify( $start_date )->getTimestamp();
		$end_date_to_time   = $now->modify( $end_date )->getTimestamp();
        $chart_interval     = multistorex_get_interval_between_dates( $start_date_to_time, $end_date_to_time, $group_by );
		...
		$orders = multistorex_get_order_report_data(
            array(
				'data' => array(
					'_order_total' => array(
						'type'     => 'meta',
		...
		$order_counts      = multistorex_prepare_chart_data( $orders, 'post_date', 'total_orders', $chart_interval, $start_date_to_time, $group_by );
		$order_amounts     = multistorex_prepare_chart_data( $orders, 'post_date', 'total_sales', $chart_interval, $start_date_to_time, $group_by );
    ...
    <script type="text/javascript">
        jQuery(function($) {
            ...
            var series = [
                {
                    label: "<?php echo esc_js( __( 'Sales total', 'multistorex-lite' ) ); ?>",
                ...
                {
                    label: "<?php echo esc_js( __( 'Number of orders', 'multistorex-lite' ) ); ?>",
                    ...
                    shadowSize: 0,
                    append_tooltip: " <?php echo esc_html__( 'Sales', 'multistorex-lite' ); ?>"
                },
            ];
