...
class Rewrites {
    ...
    public function __construct() {
        $this->custom_store_url = multistorex_get_option( 'custom_store_url', 'multistorex_general', 'store' );
        ...
        add_filter( 'template_include', array( $this, 'store_template' ), 99 );
        add_filter( 'template_include', array( $this, 'product_edit_template' ), 99 );
        add_filter( 'template_include', array( $this, 'store_toc_template' ), 99 );
        add_filter( 'query_vars', array( $this, 'register_query_var' ) );
        add_filter( 'woocommerce_get_breadcrumb', array( $this, 'store_page_breadcrumb' ) );
        add_filter( 'tiny_mce_before_init', array( $this, 'remove_h1_from_heading_in_edit_product_page' ) );
    }
    ...
    public function store_page_breadcrumb( $crumbs ) {
        ...
        $crumbs[1]   = [ ucwords( $this->custom_store_url ), get_permalink( multistorex_get_option( 'store_listing', 'multistorex_pages' ) ) ];
        $crumbs[2]   = [ $author, multistorex_get_store_url( $seller_info->data->ID ) ];
    ...
    public function register_rule() {
        ...
        do_action( 'multistorex_rewrite_rules_loaded', $this->custom_store_url );
    }
    ...
    public function resolve_wc_query_conflict( $query_vars ) {
        ...
        $dashboard_id = apply_filters( 'multistorex_get_dashboard_page_id', multistorex_get_option( 'dashboard', 'multistorex_pages', 0 ) );

        if ( ! empty( $post->ID ) && apply_filters( 'multistorex_get_current_page_id', $post->ID ) === absint( $dashboard_id ) ) {
    ...
    public function store_template( $template ) {
        ...
        if ( ! empty( $store_name ) ) {
            ...
            if ( ! multistorex_is_user_seller( $store_user->ID ) ) {
                return get_404_template();
            }

            return multistorex_locate_template( 'store.php' );
        }
    ...
    public function store_toc_template( $template ) {
        ...
        if ( get_query_var( 'toc' ) ) {
            ...
            if ( ! empty( $store_name ) ) {
                ...
                return multistorex_locate_template( 'store-toc.php' );
            }
        }
    ...
    public function product_edit_template( $template ) {
        ...
        if ( ! current_user_can( 'multistorex_edit_product' ) ) {
            return $template;
        }
        ...
        $edit_product_url = multistorex_locate_template( 'products/edit-product-single.php' );

        return apply_filters( 'multistorex_get_product_edit_template', $edit_product_url );
    }
    ...
    public function remove_h1_from_heading_in_edit_product_page( $args ) {
        ...
        if ( ( multistorex_is_seller_dashboard() && isset( $wp->query_vars['settings'] ) && $wp->query_vars['settings'] === 'store' ) || ( ! empty( $_GET['product_id'] ) && ! empty( $_GET['action'] ) && ! empty( $_GET['_multistorex_edit_product_nonce'] ) ) ) {
    ...
    public function store_query_filter( $query ) {
        ...
        if ( ! is_admin() && $query->is_main_query() && ! empty( $author ) ) {
            ...
            $store_info    = multistorex_get_store_info( $seller_info->data->ID );
            $product_ppp   = multistorex_get_option( 'store_products_per_page', 'multistorex_general', 12 );
            $post_per_page = isset( $store_info['store_ppp'] ) && ! empty( $store_info['store_ppp'] ) ? $store_info['store_ppp'] : $product_ppp;

            do_action( 'multistorex_store_page_query_filter', $query, $store_info );
            set_query_var( 'posts_per_page', apply_filters( 'multistorex_store_products_per_page', $post_per_page ) );
            ...
            $attributes = multistorex_get_chosen_taxonomy_attributes();
            if ( ! empty( $attributes ) ) {
            ...
            $query->set( 'tax_query', apply_filters( 'multistorex_store_tax_query', $tax_query ) );
    ...
    public function get_catalog_ordering_args( $orderby = '', $order = '' ) {
        ...
        if ( ! $orderby ) {
            ...
            if ( ! $orderby_value ) {
                if ( is_search() ) {
                    ...
                    $orderby_value = apply_filters( 'multistorex_default_catalog_orderby', get_option( 'woocommerce_default_catalog_orderby', 'menu_order' ) );
                }
            }
        ...
        return apply_filters( 'multistorex_get_store_products_ordering_args', $args, $orderby, $order );
    }
