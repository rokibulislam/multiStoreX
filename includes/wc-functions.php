 ...
 * @return void
 */
function multistorex_process_product_meta( int $post_id, $data = array() ) {
    if ( ! $post_id || ! $data ) {
        return;
    }
    ...
    if ( isset( $data['product_image_gallery'] ) ) {
        $data = apply_filters( 'multistorex_restrict_product_image_gallery_on_edit', $data );
    ...
    if ( 'yes' === get_option( 'woocommerce_manage_stock' ) ) {
        ...
        if ( $stock_status ) {
            try {
                ...
                multistorex_log( 'product stock update exception' );
            }
        }
    ...
    if ( 'yes' === $is_downloadable ) {
        ...
        if ( isset( $data['_wc_file_urls'] ) ) {
            ...
            do_action( 'multistorex_process_file_download', $post_id, 0, $files );
    ...
    try {
        ...
        $woocommerce_errors[] = __( 'Product SKU must be unique', 'multistorex-lite' );
    }
    ...
    do_action( 'multistorex_process_product_meta', $post_id );
 ...
 * @return void
 */
function multistorex_process_product_file_download_paths( int $product_id, int $variation_id, array $downloadable_files ) {
    global $wpdb;
    ...
    if ( ! empty( $new_download_ids ) || ! empty( $removed_download_ids ) ) {
        ...
        foreach ( $existing_permissions as $existing_permission ) {
            ...
            if ( ! empty( multistorex_get_prop( $order, 'id' ) ) ) {
                // Remove permissions
                if ( ! empty( $removed_download_ids ) ) {
                    foreach ( $removed_download_ids as $download_id ) {
                        if ( apply_filters( 'woocommerce_process_product_file_download_paths_remove_access_to_old_file', true, $download_id, $product_id, $order ) ) {
                            $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}woocommerce_downloadable_product_permissions WHERE order_id = %d AND product_id = %d AND download_id = %s", multistorex_get_prop( $order, 'id' ), $product_id, $download_id ) );
                        }
                    }
                }
                // Add permissions
                if ( ! empty( $new_download_ids ) ) {
                    foreach ( $new_download_ids as $download_id ) {
                        if ( apply_filters( 'woocommerce_process_product_file_download_paths_grant_access_to_new_file', true, $download_id, $product_id, $order ) ) {
                            ...
                            if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT 1=1 FROM {$wpdb->prefix}woocommerce_downloadable_product_permissions WHERE order_id = %d AND product_id = %d AND download_id = %s", multistorex_get_prop( $order, 'id' ), $product_id, $download_id ) ) ) {
 ...
 * @return int
 */
function multistorex_sub_order_get_total_coupon( int $order_id ) {
    global $wpdb;
 ...
 * @return string
 */
function multistorex_seller_displayname( $display_name ) {
    if ( current_user_can( 'seller' ) && ! is_admin() ) {
        $seller_info  = multistorex_get_store_info( multistorex_get_current_user_id() );
 ...
 * @return WP_Query
 */
function multistorex_get_featured_products( $per_page = 9, $seller_id = '', $page = 1 ) {
    $args = [
    ...
    return multistorex()->product->featured( apply_filters( 'multistorex_get_featured_products', $args ) );
}
 ...
 * @return WP_Query
 */
function multistorex_get_latest_products( $per_page = 9, $seller_id = '', $page = 1 ) {
    $args = [
        'posts_per_page'      => $per_page,
        'paged'               => $page,
        'post_status'         => 'publish',
    ...
    return multistorex()->product->latest( apply_filters( 'multistorex_get_latest_products', $args ) );
}
 ...
 * @param int     $seller_id seller_id
 * @param int     $page page.
 * @param boolean $hide_outofstock hide_outofstock.
 *
 * @return WP_Query
 */
function multistorex_get_best_selling_products( $per_page = 8, $seller_id = '', $page = 1, $hide_outofstock = false ) {
    $args = [
        'post_type'           => 'product',
        'post_status'         => 'publish',
        'ignore_sticky_posts' => 1,
    ...
    return multistorex()->product->best_selling( apply_filters( 'multistorex_best_selling_query', $args ) );
}
...
function check_more_seller_product_tab() {
    ...
    $store_info = multistorex_get_store_info( $post->post_author );
 ...
 * @return WP_Query
 */
function multistorex_get_top_rated_products( $per_page = 8, $seller_id = '', $page = 1 ) {
    $args = [
        'post_type'           => 'product',
        'post_status'         => 'publish',
        'ignore_sticky_posts' => 1,
    ...
    return multistorex()->product->top_rated( apply_filters( 'multistorex_top_rated_query', $args ) );
}
 ...
 * @return WP_Query
 */
function multistorex_get_on_sale_products( int $per_page = 10, int $paged = 1, int $seller_id = 0 ) {
    // Get products on sale
    ...
    return new WP_Query( apply_filters( 'multistorex_on_sale_products_query', $args ) );
}
 ...
 */
function multistorex_get_seller_balance( $seller_id, $formatted = true ) {
 ...
 *
 * @param boolean $formatted formatted.
 * @param string  $on_date on_date.
 * @param int     $seller_id seller_id.
 *
 * @return float|null
 */
function multistorex_get_seller_earnings( $seller_id, $formatted = true, $on_date = '' ) {
    $vendor = multistorex()->vendor->get( $seller_id );
 ...
 * @return array
 */
function multistorex_get_seller_rating( $seller_id ) {
    $vendor = multistorex()->vendor->get( $seller_id );
 ...
 * @return string
 */
function multistorex_get_readable_seller_rating( $seller_id ) {
    $vendor = multistorex()->vendor->get( $seller_id );
 ...
 * @return void
 */
function multistorex_exclude_child_customer_receipt( &$phpmailer ) {
    $subject = $phpmailer->Subject; ////phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase

    // order receipt
    $sub_receipt  = __( 'Your {site_title} order receipt from {order_date}', 'multistorex-lite' );
    $sub_download = __( 'Your {site_title} order from {order_date} is complete', 'multistorex-lite' );
    ...
    if ( isset( $matches[1] ) ) {
        ...
        if ( ! is_wp_error( $order ) && $order->post_parent !== 0 ) {
            $phpmailer = new multistorexFakeMailer();
        }
    }
}

add_action( 'phpmailer_init', 'multistorex_exclude_child_customer_receipt' );

/**
 * A fake mailer class to replace phpmailer
 */
class multistorexFakeMailer {
...
add_filter( 'woocommerce_dashboard_status_widget_sales_query', 'multistorex_filter_woocommerce_dashboard_status_widget_sales_query' );

/**
 * Woocommerce Admin dashboard Sales Report Synced with multistorex Dashboard report
 *
 * @param array $query query.
 *
 * @return array
 */
function multistorex_filter_woocommerce_dashboard_status_widget_sales_query( $query ) {
    global $wpdb;
 ...
 * @return void
 */
function multistorex_save_account_details() {
    if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['_wpnonce'] ) ), 'multistorex_save_account_details' ) ) {
        return;
    }
    ...
    $required_fields = apply_filters(
        'woocommerce_save_account_details_required_fields', [
            'account_first_name' => __( 'First Name', 'multistorex-lite' ),
            'account_last_name'  => __( 'Last Name', 'multistorex-lite' ),
            'account_email'      => __( 'Email address', 'multistorex-lite' ),
        ]
    );

    foreach ( $required_fields as $field_key => $field_name ) {
        if ( empty( $_POST[ $field_key ] ) ) {
            wc_add_notice( '<strong>' . esc_html( $field_name ) . '</strong> ' . __( 'is a required field.', 'multistorex-lite' ), 'error' );
        }
    }

    if ( $account_email ) {
        if ( ! is_email( $account_email ) ) {
            wc_add_notice( __( 'Please provide a valid email address.', 'multistorex-lite' ), 'error' );
        } elseif ( email_exists( $account_email ) && $account_email !== $current_user->user_email ) {
            wc_add_notice( __( 'This email address is already registered.', 'multistorex-lite' ), 'error' );
        }
        $user->user_email = $account_email;
    }

    if ( ! empty( $pass1 ) && ! wp_check_password( $pass_cur, $current_user->user_pass, $current_user->ID ) ) {
        wc_add_notice( __( 'Your current password is incorrect.', 'multistorex-lite' ), 'error' );
        $save_pass = false;
    }

    if ( ! empty( $pass_cur ) && empty( $pass1 ) && empty( $pass2 ) ) {
        wc_add_notice( __( 'Please fill out all password fields.', 'multistorex-lite' ), 'error' );
        $save_pass = false;
    } elseif ( ! empty( $pass1 ) && empty( $pass_cur ) ) {
        wc_add_notice( __( 'Please enter your current password.', 'multistorex-lite' ), 'error' );
        $save_pass = false;
    } elseif ( ! empty( $pass1 ) && empty( $pass2 ) ) {
        wc_add_notice( __( 'Please re-enter your password.', 'multistorex-lite' ), 'error' );
        $save_pass = false;
    } elseif ( ( ! empty( $pass1 ) || ! empty( $pass2 ) ) && $pass1 !== $pass2 ) {
        wc_add_notice( __( 'New passwords do not match.', 'multistorex-lite' ), 'error' );
        $save_pass = false;
    }
    ...
    if ( wc_notice_count( 'error' ) === 0 ) {
        ...
        wc_add_notice( __( 'Account details changed successfully.', 'multistorex-lite' ) );

        do_action( 'woocommerce_save_account_details', $user->ID );

        wp_safe_redirect( multistorex_get_navigation_url( 'edit-account' ) );
        exit;
    }
}

add_action( 'template_redirect', 'multistorex_save_account_details' );
 ...
 * @return void
 */
function multistorex_clear_best_selling_product_category_cache( $order_id ) {
    ...
    $seller_id = multistorex_get_seller_id_by_order( $order_id );
    if ( empty( $seller_id ) ) {
        return;
    }

    delete_transient( 'multistorex_vendor_get_best_selling_products_' . $seller_id );
    delete_transient( 'multistorex_vendor_get_best_selling_categories_' . $seller_id );
}

add_action( 'woocommerce_new_order', 'multistorex_clear_best_selling_product_category_cache', 10, 1 );
add_action( 'woocommerce_update_order', 'multistorex_clear_best_selling_product_category_cache', 10, 1 );
 ...
 * @return void
 */
function multistorex_clear_edit_product_category_cache( $term_id ) {
    // get taxonomy slug
    ...
    foreach ( $seller_ids as $seller_id ) {
        ...
        if ( function_exists( 'wpml_get_active_languages' ) ) {
            foreach ( wpml_get_active_languages() as $active_language ) {
                delete_transient( 'multistorex_vendor_get_store_categories_' . $active_language['code'] . '_' . $seller_id );
            }
        }

        delete_transient( 'multistorex_vendor_get_store_categories_' . $seller_id );
    }
}

add_action( 'edit_product_cat', 'multistorex_clear_edit_product_category_cache', 10, 1 );
add_action( 'pre_delete_term', 'multistorex_clear_edit_product_category_cache', 10, 1 );

/**
 * Remove banner when without banner layout selected for profile
 *
 * @param array $progress_values progress_values.
 *
 * @return array
 */
function multistorex_split_profile_completion_value( $progress_values ) {
    $store_banner = multistorex_get_option( 'store_header_template', 'multistorex_appearance' );
...
add_filter( 'multistorex_profile_completion_values', 'multistorex_split_profile_completion_value', 10 );

/**
 * Set More products from seller tab on Single Product Pagess
 *
 * @param array $tabs tabs.
 *
 * @return array
 */
function multistorex_set_more_from_seller_tab( $tabs ) {

    if ( check_more_seller_product_tab() ) {
    
        $tabs['more_seller_product'] = [
            'title'    => __( 'More Products', 'multistorex-lite' ),
            'priority' => 99,
            'callback' => 'multistorex_get_more_products_from_seller',
        ];
    }
...
add_action( 'woocommerce_product_tabs', 'multistorex_set_more_from_seller_tab', 10 );
 ...
 * @return void
 */
function multistorex_get_more_products_from_seller( $seller_id = 0, $posts_per_page = 6 ) {
    global $product, $post;
    ...
    if ( ! is_int( $posts_per_page ) ) {
        $posts_per_page = apply_filters( 'multistorex_get_more_products_per_page', 6 );
    }
    ...
    if ( $products->have_posts() ) {
        ...
        esc_html_e( 'No product has been found!', 'multistorex-lite' );
    }
 ...
 * @return void
 */
function multistorex_bulk_order_status_change() {
    if ( ! current_user_can( 'multistorex_manage_order' ) ) {
        return;
    }

    if ( multistorex_get_option( 'order_status_change', 'multistorex_selling' ) === 'off' ) {
        return;
    }
    ...
    multistorex_apply_bulk_order_status_change(
        [
...
add_action( 'template_redirect', 'multistorex_bulk_order_status_change' );
 ...
 * @return string
 */
function multistorex_add_reply_to_vendor_email_on_wc_customer_note_mail( $headers, $id, $order ) {
...
add_filter( 'woocommerce_email_headers', 'multistorex_add_reply_to_vendor_email_on_wc_customer_note_mail', 10, 3 );
 ...
 * @return void
 */
function multistorex_keep_old_vendor_woocommerce_duplicate_product( $duplicate, $product ) {
    ...
    multistorex_override_product_author( $duplicate, absint( $old_author ) );
}

add_action( 'woocommerce_product_duplicate', 'multistorex_keep_old_vendor_woocommerce_duplicate_product', 35, 2 );
...
function send_email_for_order_cancellation( $recipient, $order ) {
    ...
    $seller_id = multistorex_get_seller_id_by_order( $order->get_id() );
 ...
 */
function multistorex_modify_vendor_order_counts( $counts ) {
    global $pagenow;
    ...
    $vendor_order_counts = multistorex_count_orders( $vendor_id );
...
add_filter( 'wp_count_posts', 'multistorex_modify_vendor_order_counts', 10, 1 );


/**
 * vendor own product purchase restriction 
 * 
 ...
 * @return boolean
 */
function multistorex_vendor_own_product_purchase_restriction( bool $is_purchasable, $product ): bool {
    if ( false === $is_purchasable || multistorex_is_product_author( $product->get_id() ) ) {
        $is_purchasable = false;
    }
    return $is_purchasable;
}

add_filter( 'woocommerce_is_purchasable', 'multistorex_vendor_own_product_purchase_restriction', 10, 2 );
 ...
 * @return array
 */
function multistorex_vendor_product_review_restriction( array $data ) {
    global $product;
    ...
    if ( multistorex_is_product_author( $product->get_id() ) ) {
        $data['title_reply']   = __( 'Reviews cannot be posted for products that you own.', 'multistorex-lite' );
        $data['comment_field'] = '';
        $data['fields']        = array();
        $data['submit_field']  = '';
        $data['submit_button'] = '';
...
add_filter( 'woocommerce_product_review_comment_form_args', 'multistorex_vendor_product_review_restriction' );