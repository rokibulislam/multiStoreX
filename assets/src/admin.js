...
jQuery(function($) {
    ...
    $('.multistorex-settings-radio-image button').on('click', function (e) {
        e.preventDefault();

        var btn = $(this),
            template = btn.data('template'),
            input = btn.data('input'),
            container = btn.parents('.multistorex-settings-radio-image-container');

        $('#' + input).val(template);

        container.find('.active').removeClass('active').addClass('not-active');

        btn.parents('.multistorex-settings-radio-image').addClass('active').removeClass('not-active');
    });
    ...
    $( document.body )
        ...
        .on( 'wc_add_error_tip', function( e, element, error_type ) {
            ...
            if ( element.parent().find( '.wc_error_tip' ).length === 0 ) {
                element.after( '<div class="wc_error_tip ' + error_type + '">' + multistorex[error_type] + '</div>' );
        ...
        .on(
            ...
            function() {
                ...
                if ( $( this ).is( '.wc_input_price' ) || $( this ).is( '#refund_amount' ) ) {
                    ...
                    regex = new RegExp( '[^\-0-9\%\\' + multistorex.mon_decimal_point + ']+', 'gi' );
                    decimalRegex = new RegExp( '[^\\' + multistorex.mon_decimal_point + ']', 'gi' );
        ...
        .on( 'change', '#_sale_price.wc_input_price[type=text], .wc_input_price[name^=variable_sale_price]', function() {
            ...
            var sale_price    = parseFloat(
                window.accounting.unformat( sale_price_field.val(), multistorex.mon_decimal_point )
            );
            var regular_price = parseFloat(
                window.accounting.unformat( regular_price_field.val(), multistorex.mon_decimal_point )
            );
